...
class NewsFilesAPI {
  ...
  async get_news_files(
    ...
    const result = fetch(
      `${cddBaseURL}api/news/get-news-files/?${page ? 'page=' + page : ''}${page_size ? '&page_size=' + page_size : ''}${administrativelevel_id ? '&administrativelevel_id=' + administrativelevel_id : ''}${cvd_id ? '&cvd_id=' + cvd_id : ''}${project_id ? '&project_id=' + project_id : ''}`,
      requestOptions,
    )
